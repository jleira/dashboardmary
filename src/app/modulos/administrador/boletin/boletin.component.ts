...
export class BoletinComponent implements OnInit {
  ...
  cargarnoticias() {
    this.api.getdata('allBoletines').subscribe((items) => {
  ...
  eliminaritem(id) {
    Swal.fire({
      ...
      if (result.value) {
        ...
        this.api.getdata('deleteBoletines/' + id).subscribe((items) => {
  ...
  crear() {
    ...
    this.api.postdata(formData, 'newBoletines').subscribe((respuesta) => {
  ...
  editarpublicacion() {
    ...
    this.api.postdata(formData, 'editBoletines').subscribe((respuesta) => {
      this.api.notificaciones('success', 'Información cargada exitosamente');
        this.cargarnoticias();
      this.datos_crear = {
        titulo_espanol: ''
